...
class DownBlock2d(nn.Module):
    ...
    def __init__(self, in_features, out_features, norm=False, kernel_size=4, pool=False, sn=False):
        super().__init__()
        self.conv = nn.Conv2d(in_features, out_features, kernel_size=kernel_size, padding=2)
        if sn:
...
class Discriminator(nn.Module):
    ...
    def __init__(self, channel_basic=64, num_blocks=4, max_feature=512, sn=True):
        super().__init__()
        ...
        for i in range(num_blocks):
            down_blocks.append(
                DownBlock2d(3 if i == 0 else min(max_feature, channel_basic * (2 ** i)),
                            ...
                            norm=(i != 0), kernel_size=4, pool=True, sn=sn)
            )
